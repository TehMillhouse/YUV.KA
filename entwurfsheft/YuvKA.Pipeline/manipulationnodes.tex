...
\paragraph{Beschreibung}~\\
Diese Klasse\name{BrightnessContrastSaturationNode} ändert die Helligkeit, den Kontrast oder die Farbsättigung des \name{Frame}s.
...
\method{ProcessFrame}
	...
	public override Frame[] ProcessFrame(Frame[] inputs, int frameIndex)
	\end{verbatim}
	Setzt die Helligkeit, den Kontrast und die Farbsättigung des \name{Frame}s auf die gegebenen Werte. Dafür werden die Farbkanäle jedes Pixels aus dem RGB-Raum in den HSV-Raum umgerechnet, die Änderungen vorgenommen und anschließend wieder nach RGB konvertiert. Es werden folgende Algorithmen für die Konvertierungen benutzt:
	\floatname{algorithm}{Algorithmus}
	\begin{algorithm}
	\caption{RGB nach HSV Konvertierung}
		\begin{algorithmic}[1]
		\STATE \COMMENT{Algorithm1 goes here}
		\end{algorithmic}
	\end{algorithm}
	...
	\caption{HSV nach RGB Konvertierung}
		\begin{algorithmic}[1]
		\STATE \COMMENT{Algorithm2 goes here}
		\end{algorithmic}
	\end{algorithm}
\end{itemize}

...
\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.85\textheight,angle=90]{classdiagram/pipe-imp.png}
\end{center}
\end{figure}
...
\begin{itemize}
	...
	\add \verb!static bool SimilarColor(Color c1, Color c2)! \\
	Entscheidet ob die zwei angegebenen Farben ähnlich sind, indem es deren Helligkeit und Sättigung vergleicht.
\end{itemize}

\paragraph{\name{+ DiagramGraphViewModel}}~\\
\name{DiagramGraphViewModel} repräsentiert das Objekt, mit dem die Einstellungen des Graphen im Ausgabefenster verwaltet werden.
\begin{itemize}
	\add \verb!public DiagramViewModel Parent { get; }! \\
	Ruft das DiagramViewModel, das diesem DiagramGraphViewModel zugrunde liegt, ab.
	
	\add \verb!public DiagramGraph Model { get; private set; }! \\
	Ruft den Diagrammgraphen ab oder setzt ihn fest.
	\add \verb!public string Name { get { return Parent.GetVideoName(Model.Video); } }! \\
	Ruft den Namen des Videos des Graphen ab.
	\add \verb!public IEnumerable<GraphTypeViewModel> AvailableTypes! \\
	Ruft die mit den momentanen Einstellungen verfügbaren Graphentypen ab oder setzt sie fest.
	\add \verb!public GraphTypeViewModel CurrentType! \\
...
\name{GraphTypeViewModel} repräsentiert das Objekt, mit dem die Graphentypen im Ausgabefenster angezeigt werden.
\begin{itemize}
...
\begin{itemize}
	...
	Diese Methode benachrichtigt alle anderen Objekte dass sich diese Property geändert hat. Dies ist notwendig damit sich die \name{View} aktualisieren kann, sobald sich der Wert einer Property ändert.
...
\begin{itemize}
	\change Wurde zu \name{NullableDoublePropertyViewModel} umbenannt, da der zugrundeliegende Typ zum \name{nullable double} gemacht wurde.
...
\name{OutputWindowViewModelPropertyViewModel} erlaubt es Wiedergabeknoten, beliebige ViewModels selbst zu liefern, welche dann vom Programm gelesen und angezeigt werden.
